...
If all client/server communication is to be routed through the Trino Gateway, 
...
to the Trino cluster on the DNS/network level, these following calls might not work
at all for users.
...
To prevent Trino Gateway from sending `X-Forwarded-*` headers, set
```yaml
routing:
